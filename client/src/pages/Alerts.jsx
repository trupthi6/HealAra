...
export default function Alerts() {
  ...
  return (
    <div className="space-y-6">
      {/* Header Panel */}
      <div>
        <h1 className="text-2xl font-bold text-gray-900 leading-tight">Clinical Alerts</h1>
        <p className="text-sm text-gray-500 mt-1">Review threshold breaches and logs flagged by the anomaly detector.</p>
      </div>
      ...
      <div className="flex border-b border-gray-200">
        ...
        >
          ...
          {resolvedAlerts.length > 0 && (
            <span className="bg-gray-100 text-gray-600 rounded-full text-[10px] px-2 py-0.5 font-bold border">
      ...
      {loading ? (
        <div className="space-y-4">
          <div className="bg-white border rounded-xl p-5 h-20 animate-shimmer"></div>
          <div className="bg-white border rounded-xl p-5 h-20 animate-shimmer"></div>
        </div>
        ...
        <div className="space-y-4">
          {currentAlerts.map((alert) => (
            <div 
              key={alert._id} 
              className={`bg-white border rounded-xl p-5 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-all ${
                alert.resolved ? 'border-gray-200 bg-gray-50/50' : 'border-red-100 hover:border-red-200'
              }`}
            >
              <div className="flex items-start gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-sm font-medium text-gray-800 leading-normal">{alert.message}</p>
              ...
              {!alert.resolved && (
                <button
                  ...
                  className="inline-flex items-center justify-center gap-2 bg-white hover:bg-teal-50 text-teal-600 border border-teal-200 hover:border-teal-300 font-semibold text-xs px-4 py-2 rounded-lg transition-colors shrink-0 self-start sm:self-center"
                >
